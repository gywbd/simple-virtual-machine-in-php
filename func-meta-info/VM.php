...
class VM {
    ...
    public function __construct($code, $startip, $nglobals, $metadata=null) {
        $this->code = $code;
        ...
        $this->callstack = array_fill(0,self::DEFAULT_CALL_STACK_SIZE,null);
        ...
        while($opcode != HALT && $this->ip < count($this->code)) {
            ...
            switch($opcode) {
                ...
                case STORE :
                    ...
                    $this->callstack[$this->callsp]->locals[$regnum] = $this->stack[$this->sp--];
                    break;
                case GSTORE :
                    $addr = $this->code[$this->ip++];
                ...
                case CALL :
                    ...
                    $nlocals = $this->metadata[$findex]->nlocals;
                    $this->callstack[++$this->callsp] = new Context($this->ip, $nargs + $nlocals);

                    //copy args into new context
                    for($i=0; $i < $nargs; $i++) {
                        $this->callstack[$this->callsp]->locals[$i] = $this->stack[$this->sp - $i];
                    }
    ...
    private function load() {
        $regnum = 0;
        $regnum = $this->code[$this->ip++];
        $this->stack[++$this->sp] = $this->callstack[$this->callsp]->locals[$regnum];
    }
